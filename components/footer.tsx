import { FaInstagram } from "react-icons/fa";

export default function Footer() {
  return (
    <footer className="bg-accent-1 border-t border-accent-2 mt-16">
        <div className="py-10 flex flex-col lg:flex-row items-center">
          <h3 className="text-4xl lg:text-5xl font-bold tracking-tighter leading-tight text-center mb-10 lg:mb-0 lg:pr-4 lg:w-1/2">
            Sadondeko NFT Project.
          </h3>
          <div className="flex flex-col lg:flex-row justify-center items-center lg:pl-4 lg:w-1/2">
            <a
              href="https://instagram.com/sadondeko_nft"
              className="flex justify-between gap-2 mx-3 bg-black hover:bg-white hover:text-black border border-black text-white font-bold py-3 px-12 lg:px-8 duration-200 transition-colors mb-6 lg:mb-0"
            >
              <FaInstagram size={30} />
              <span>Instagram</span>
            </a>
          </div>
        </div>
        <p className="text-end">copyright © Sadondeko NFT project. all rights reserved.</p>
    </footer>
  );
}
